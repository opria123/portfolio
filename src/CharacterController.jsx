...
export default function CharacterController(props) {
  const keyDownHandler = event => {
    ...
    if (event.key === 'Enter') {
      ...
      var newurl = window.location.protocol + "//" + window.location.host + window.location.pathname + '?showModal=true';
      window.history.pushState({ path: newurl }, '', newurl);
      setSearchParams(window.location.search);
    }
  };
  const { context, updateState } = useContext(Context);
  ...
  const menuEventHandler = event => {
    if (event.key === 'Backspace') {
      ...
      var newurl = window.location.protocol + "//" + window.location.host + window.location.pathname + '?showSettingsModal=true';
      window.history.pushState({ path: newurl }, '', newurl);
      setSearchParams(window.location.search);
    }
  };

  document.addEventListener('keydown', menuEventHandler);
    ...
    if (rayHit && canJump) {
      if (rayHit.collider.parent()) {
        ...
        if (
          ...
          // var newurl = window.location.protocol + "//" + window.location.host + window.location.pathname + `?content=${rayHit.collider.parent().userData}`;
          // window.history.pushState({ path: newurl }, '', newurl);
          // setSearchParams(window.location.search);
          if (!cratedEventListner) {
            console.log("REGISTERING EVENT LISTNER")
            console.log(keyDownHandler)
            cratedEventListner = true
