...
export default function CharacterController(props) {
  ...
  const isAbleToOpen = useGame((state) => state.isAbleToOpen);
  const isMenuAbleToOpen = useGame((state) => state.isMenuAbleToOpen);
  ...
  useEffect(() => {
    ...
    if (!isAbleToOpen) {
      window.addEventListener('keydown', menuEventHandler);
      return () => window.removeEventListener("keydown", menuEventHandler);
    }

    const keyDownHandler = event => {
      console.log('User pressed: ', event.key);
      if (event.key === 'Enter') {
        window.removeEventListener("keydown", menuEventHandler);
        event.preventDefault();
        document.exitPointerLock();
        setShowModal(true);
      }
  ...
  const setIsAbleToOpen = useGame((state) => state.setIsAbleToOpen);
  ...
  const setModalTarget = useGame((state) => state.setModalTarget);
    ...
    if (rayHit && canJump) {
      if (rayHit.collider.parent()) {
        ...
        if (
          ...
          setModalTarget(rayHit.collider.parent().userData)
        } else {
